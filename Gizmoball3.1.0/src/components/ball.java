...
public class ball extends JComponent implements gizmosInterface{
	...
	private ArrayList<gizmosInterface> gizmos;								//temp buffer to store all the gizmos passed in my the window
	...
	public double[] timeUntilNextCollision(ArrayList<gizmosInterface> gizmos){
		ball=new Circle(x,y,radius);
		//Vect velocity= new Vect(vx,vy);
		//ArrayList<gizmosInterface> gizmos = this.window.getGizmos();

		double[] outPut={Configuration.delta_t+1,0,0};
		double[] nextCollision={Configuration.delta_t+1,0,0};
		for(int i = 0;i<gizmos.size();i++){     	
			if(gizmos.get(i).boundingBox().intersects(this.boundingBox().x-4*Configuration.L,
					this.boundingBox().y-4*Configuration.L,8*Configuration.L,8*Configuration.L))
			{       	        			
				outPut = gizmos.get(i).timeUntilNextCollision(ball, velocity);
	...
	public void handleCollision(int collisionGizmo, int collisionSide){
		...
		velocity = gizmos.get(collisionGizmo).handleCollision(ball, velocity, collisionSide);

		if(gizmos.get(collisionGizmo).getType()=='A'){
			this.radius=0;
		}
	}
